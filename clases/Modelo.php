...
//		$this->db->query("SET SESSION sql_mode = 'TRADITIONAL'");
	...
	/**
	 *
	 * Valida si el usuario esta registrado y define a que seccion entra
	 *
	 */
	function validarLogin($usuario,$clave){
		$this->arrUsuarioLogueado=array();
		$sql = "select count(*)as cuantos from bp_usuarios where usuario ='".$usuario."' and clave='".$clave."'";
		$resultado = $this->db->query($sql);
		$datos = $resultado->fetch_assoc();
		if($datos['cuantos']==1) {
			$sql = "select * from bp_usuarios where usuario ='" . $usuario . "' and clave='" . $clave . "'";
			$resultado = $this->db->query($sql);
			$datos = $resultado->fetch_assoc();

			$this->arrUsuarioLogueado['id'] = $datos['id'];
			$this->arrUsuarioLogueado['nombre'] = $datos['nombres'] . " " . $datos['paterno'] . " " . $datos['materno'];
			$this->arrUsuarioLogueado['correo'] = $datos['correo'];
			$this->arrUsuarioLogueado['rol'] = $datos['rol'];
			$this->arrUsuarioLogueado['activadorId'] = $datos['activadorId'];


//			$this->arrUsuarioLogueado['companiaId'] = $datos['companiaId'];


			if ($this->arrUsuarioLogueado['rol'] == 'compania') {
				$this->compania = new Compania($this->db,$datos['companiaId']);
//				print "<pre>";
//				print "datos compania";
//				print_r($this->compania->datosCompania);
//				print "</pre>";

//				$sql2 = "select * from bp_companias where id=" . $this->arrUsuarioLogueado['companiaId'];
//				$resultado2 = $this->db->query($sql2);
//				$datos2 = $resultado2->fetch_assoc();
//				$this->arrUsuarioLogueado['compania'] = $datos2['nombre'];
//				$this->arrUsuarioLogueado['latitud'] = $datos2['latitud'];
//				$this->arrUsuarioLogueado['longitud'] = $datos2['longitud'];

			} else if ($this->arrUsuarioLogueado['rol'] == 'enlace') {
				$this->arrUsuarioLogueado['compania'] = '';
				$this->arrUsuarioLogueado['latitud'] = '';
				$this->arrUsuarioLogueado['longitud'] = '';
				$sql2 = "select bp_usuarios.nombres,bp_usuarios.paterno,bp_usuarios.materno,bp_usuarios.correo,
						bp_usuarios.rol,bp_usuarios.id,bp_usuarios.companiaId,bp_companias.nombre as compania,
						bp_companias.latitud as latitud,bp_companias.longitud as longitud
						from bp_usuarios
						left join bp_companias on bp_companias.id=bp_usuarios.companiaId
						where activadorId=" . $this->arrUsuarioLogueado['id'];
				$resultado2 = $this->db->query($sql2);
				$arrTmp = array();
				while ($datos2 = $resultado2->fetch_assoc()) {
					$arrTmp[] = array(
						'id' => $datos2['id'],
						'nombre' => $datos2['nombres'] . " " . $datos2['paterno'] . " " . $datos2['materno'],
						'correo' => $datos2['correo'],
						'rol' => $datos2['rol'],
						'companiaId' => $datos2['companiaId'],
						'activadorId' => $this->arrUsuarioLogueado['id'],
						'compania' => $datos2['compania'],
						'latitud' => $datos2['latitud'],
						'longitud' => $datos2['longitud'],
					);
				}
				$this->arrUsuarioLogueado['supervisados'] = $arrTmp;
			} else if ($this->arrUsuarioLogueado['rol'] == 'admin') {

			}
//			print "<pre>";
//			print "arrUsuario";
//			print_r($this->arrUsuarioLogueado);
//			print "</pre>";
		}else{
	...
	function hacerArreglosBase()
	{
		...
		print "<pre>";
		echo "<br>buenas practicas  <BR>";
		print_r($this->arrBuenasPracticas);
		print "</pre>";
	}
